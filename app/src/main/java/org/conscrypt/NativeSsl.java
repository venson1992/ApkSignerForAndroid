...
public final class NativeSsl {
    ...
    static NativeSsl newInstance(SSLParametersImpl parameters2, NativeCrypto.SSLHandshakeCallbacks handshakeCallbacks2, SSLParametersImpl.AliasChooser chooser, SSLParametersImpl.PSKCallbacks pskCallbacks2) throws SSLException {
        ...
        return new NativeSsl(NativeCrypto.SSL_new(ctx.sslCtxNativePointer, ctx), parameters2, handshakeCallbacks2, chooser, pskCallbacks2);
    }
    ...
    public byte[] exportKeyingMaterial(String label, byte[] context, int length) throws SSLException {
        ...
        return NativeCrypto.SSL_export_keying_material(this.ssl, this, label.getBytes(Charset.forName("US-ASCII")), context, length);
    }
    ...
    public int serverPSKKeyRequested(String identityHint, String identity, byte[] key) {
        byte[] secretKeyBytes;
        PSKKeyManager pskKeyManager = this.parameters.getPSKKeyManager();
        if (pskKeyManager == null || (secretKeyBytes = this.pskCallbacks.getPSKKey(pskKeyManager, identityHint, identity).getEncoded()) == null || secretKeyBytes.length > key.length) {
            return 0;
        }
        System.arraycopy(secretKeyBytes, 0, key, 0, secretKeyBytes.length);
        return secretKeyBytes.length;
    ...
    /* access modifiers changed from: package-private */
    public void chooseClientCertificate(byte[] keyTypeBytes, int[] signatureAlgs, byte[][] asn1DerEncodedPrincipals) throws SSLException, CertificateEncodingException {
    ...
    private void setCertificate(String alias) throws CertificateEncodingException, SSLException {
        ...
        if (alias != null && (keyManager = this.parameters.getX509KeyManager()) != null && (privateKey = keyManager.getPrivateKey(alias)) != null) {
    ...
    public void configureServerCertificate() throws IOException {
        ...
        if (!isClient() && (keyManager = this.parameters.getX509KeyManager()) != null) {
            for (String keyType : getCipherKeyTypes()) {
                try {
                    setCertificate(this.aliasChooser.chooseServerAlias(keyManager, keyType));
    ...
    private void verifyWithSniMatchers(String serverName) throws SSLHandshakeException {
        if (AddressUtils.isValidSniHostname(serverName) && !Platform.serverNamePermitted(this.parameters, serverName)) {
    ...
    private void setCertificateValidation() throws SSLException {
        ...
        if (!isClient()) {
            ...
            if (certRequested && (issuers = this.parameters.getX509TrustManager().getAcceptedIssuers()) != null && issuers.length != 0) {
                try {
                    NativeCrypto.SSL_set_client_CA_list(this.ssl, this, SSLUtils.encodeSubjectX509Principals(issuers));
                } catch (CertificateEncodingException e) {
